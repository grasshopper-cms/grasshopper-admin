...
define(['grasshopperBaseView', 'underscore', 'jquery', 'require', 'resources'],
    function (GrasshopperBaseView, _, $, require, resources) {
        ...
        function beforeRender($deferred) {
            var self = this;
            self.model.get('users').fetch().fail(function (jqXHR, status, error) {
        ...
        function afterRender() {
            var value = this.model.get('value'), selectEl = this.$('select');
            selectEl.select2({
        ...
        function _getValueStruct(id) {
            ...
            if (foundUser){
                return {id: id, displayName: foundUser.get('displayName')};
            }
